...
export default function AvgHours({ selectedMemberId, average }: Prop) {
  ...
  useEffect(() => {
    const fetchUserTasks = async () => {
      if (selectedMemberId != 0) {
        try {
          ...
          const taskResponse = await taskService.getUserCountOfTasks(
            selectedMemberId
          );
    ...
    fetchUserTasks()
  }, [selectedMemberId != 0 && userTasks != 0]);
    ...
    <div className="text-center">
      {selectedMemberId == 0 ? (
        ...
        <div>
          ...
          <p className="text-black text-9xl font-bold mt-1">{userTasks}</p>
